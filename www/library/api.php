...
function ShowBattle($battle) {
  ...
  echo '<tr><td nowrap>マップ</td><td>' . $battle['map_key'] . ' ' . ($battle['map_extensions'] ? '(拡張あり)' : '') . "</td></tr>\n";
  echo '<tr><td nowrap>作成時刻</td><td>' . $battle['battle_created'] . "</td></tr>\n";
  echo '<tr><td nowrap>更新時刻</td><td>' . $battle['battle_modified'] . "</td></tr>\n";
  if (isset($battle['battle_log_info']) && $battle['battle_log_info'] != '') {
    echo '<tr><td nowrap>エラー</td><td><pre style="margin:0;white-space: pre-wrap;">' . htmlspecialchars($battle['battle_log_info']) . '</pre></td></tr>';
  }
  echo '<tr><td>結果</td><td>';
